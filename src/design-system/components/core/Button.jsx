...
export function Button({variant='primary',size='md',pill=false,glow=false,disabled=false,children,style,...rest}){
  ...
  const variants={
    primary:{background:'var(--accent-grad)',color:'var(--text-on-accent)',boxShadow:'var(--shadow-accent)'},
    secondary:{background:'var(--surface-card)',color:'var(--text-primary)',boxShadow:'var(--shadow-sm)',border:'1px solid var(--gray-200)'},
    soft:{background:'var(--accent-soft)',color:'var(--gold-600)'},
  ...
  return <button style={s} disabled={disabled}
    onMouseEnter={e=>{if(variant==='primary')e.currentTarget.style.background='var(--accent-grad-hover)';if(variant==='secondary'||variant==='ghost')e.currentTarget.style.background='var(--gray-050)'}}
