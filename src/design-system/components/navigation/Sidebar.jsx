...
export function Sidebar({brand='CL',brandLabel='Content-Leads',items=[],activeId,onSelect,footer,promo,width=250,style}){
  return <div style={{width,background:'var(--surface-card)',borderRight:'1px solid var(--gray-100)',display:'flex',flexDirection:'column',padding:'16px 12px',boxSizing:'border-box',fontFamily:'var(--font-ui)',...style}}>
    {brandLabel&&<div style={{display:'flex',alignItems:'center',gap:10,padding:'4px 8px 16px',borderBottom:'1px solid var(--gray-100)',marginBottom:12}}>
      <div style={{width:34,height:34,borderRadius:10,background:'var(--accent-grad)',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:600,fontSize:15,color:'#fff',boxShadow:'var(--shadow-accent)'}}>{brand}</div>
    ...
    <div style={{display:'flex',flexDirection:'column',gap:4}}>
      {items.map(it=>{
        ...
        return <button key={it.id} onClick={()=>onSelect&&onSelect(it.id)} style={{display:'flex',alignItems:'center',gap:12,padding:'11px 14px',borderRadius:'var(--radius-md)',border:active?'1px solid var(--gold-200)':'1px solid transparent',background:active?'var(--accent-soft)':'transparent',color:active?'var(--gold-600)':'var(--text-primary)',font:'500 15px var(--font-ui)',letterSpacing:'-0.01em',cursor:'pointer',textAlign:'left',transition:'background var(--dur-fast) var(--ease-out)'}}
