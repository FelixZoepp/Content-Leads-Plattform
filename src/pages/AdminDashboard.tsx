    ...
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[#94A3B8] mt-0.5">Portfolio-Gesamtübersicht · {tenants.length} aktive Kunden</p>
        </div>
      ...
      {/* Alerts */}
      {alerts.length > 0 && (
        <div className="bg-[#111827] border border-[#1E293B] rounded-2xl">
          ...
          <div className="divide-y divide-[#1E293B]/50">
            {alerts.slice(0, 5).map((alert: any) => (
              <div key={alert.id} className="px-5 py-3 flex items-center justify-between hover:bg-[#1A2235] transition">
                <div className="flex items-center gap-3">
                  <div className="w-2 h-2 rounded-full bg-[#EF4444]" />
                  <div>
                    <p className="text-sm text-white">{alert.tenants?.company_name || "Unbekannt"}</p>
                    <p className="text-xs text-[#94A3B8]">{alert.message || "Health Score kritisch"}</p>
                  </div>
                </div>
      ...
      <div className="bg-[#111827] border border-[#1E293B] rounded-2xl">
        <div className="px-5 py-4 border-b border-[#1E293B] flex items-center justify-between">
          ...
          <span className="text-xs text-[#94A3B8]">{tenants.length} Kunden</span>
        </div>
        {tenants.length === 0 ? (
          <div className="p-12 text-center">
            <Users className="w-12 h-12 text-[#1E293B] mx-auto mb-3" />
            <p className="text-sm text-[#94A3B8] mb-1">Noch keine Kunden</p>
            <p className="text-xs text-[#64748B]">Lade deinen ersten Kunden ein um loszulegen</p>
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#1E293B]">
                <th className="px-5 py-3 text-left text-xs font-medium text-[#94A3B8]">Unternehmen</th>
                <th className="px-5 py-3 text-left text-xs font-medium text-[#94A3B8]">Kontakt</th>
                <th className="px-5 py-3 text-left text-xs font-medium text-[#94A3B8]">Health Score</th>
                <th className="px-5 py-3 text-left text-xs font-medium text-[#94A3B8]">Seit</th>
                <th className="px-5 py-3 text-right text-xs font-medium text-[#94A3B8]"></th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#1E293B]/50">
              {tenants.map((tenant) => (
                <tr key={tenant.id} className="hover:bg-[#1A2235] transition cursor-pointer">
                  ...
                  <td className="px-5 py-3 text-[#94A3B8]">{tenant.contact_name || "—"}</td>
                  <td className="px-5 py-3">
                    {tenant.latestHealth ? (
                      <HealthBadge color={tenant.latestHealth.color} score={tenant.latestHealth.score} />
                    ) : (
                      <span className="text-xs text-[#64748B]">Nicht berechnet</span>
                    )}
                  </td>
                  ...
                  <td className="px-5 py-3 text-right">
                    <ChevronRight className="w-4 h-4 text-[#64748B] inline" />
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {showInvite && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setShowInvite(false)}>
          <div className="bg-[#111827] border border-[#1E293B] rounded-2xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
            <h2 className="text-lg font-semibold text-white mb-1">Neuen Kunden einladen</h2>
            <p className="text-sm text-[#94A3B8] mb-6">Der Kunde erhält eine E-Mail mit Login-Daten.</p>
            <div className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={() => setShowInvite(false)}
                  className="flex-1 px-4 py-2.5 border border-[#1E293B] rounded-lg text-sm text-[#94A3B8] hover:bg-[#1A2235] transition">
                  Abbrechen
                </button>
