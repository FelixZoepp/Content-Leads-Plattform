  ...
  return (
    <div className="space-y-6 max-w-6xl">
      <div className="flex items-center justify-between">
        ...
        <button className="flex items-center gap-2 bg-[#4A9FD9] hover:bg-[#2E7BB5] text-white font-medium px-4 py-2 rounded-lg transition text-sm">
      ...
      <div className="bg-[#12121A] border border-[#2A2A35] rounded-2xl p-6">
        ...
        <div className="grid grid-cols-7 gap-1 mb-2">
          {days.map(d => (
            <div key={d} className="text-center text-xs text-[#555566] py-2">{d}</div>
          ))}
        </div>
          ...
          {Array.from({ length: 35 }).map((_, i) => {
            ...
            return (
              <div key={i} className={`aspect-square flex items-center justify-center rounded-lg text-sm cursor-pointer transition ${
                isToday ? "bg-[#4A9FD9] text-white font-semibold" :
                inMonth ? "text-[#8888AA] hover:bg-[#1A1A25]" : "text-[#2A2A35]"
              }`}>
                {inMonth ? day : ""}
              </div>
            );
